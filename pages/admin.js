...
import { userData } from '../utils/test.js';
...
export class AdminPage{
    ...
    async perform(){
        ...
        await this.page.click(this.option(userData.userRole));   

        await this.page.click(this.dropdown2);
        await this.page.click(this.option(userData.status));     
    }

    //async dynamic(){
        //await this.page.fill(this.input, userData.employeeName[0]);  
        //await this.page.click(this.suggestion(userData.employeeName));     
    //}
     async dynamic() {
    await this.page.fill(this.input, "a");

    const first = this.page.locator("//div[@role='listbox']//span").first();
    await first.waitFor();

    const empName = (await first.textContent()).trim();
...
async fillForm(){
        await this.page.fill(this.input2, userData.username);              
        await this.page.fill(this.pass, userData.password);                
        await this.page.fill(this.passc, userData.confirmPassword);        
